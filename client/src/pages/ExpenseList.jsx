...
const ExpenseList = () => {
  ...
  return isExpensesLoading ||
    (isExpensesFetching && isRemindersFetching) ||
    isRemindersLoading ? (
    <div>
    ...
    <div className=" text-white  min-h-screen">
      <main className="p-5 flex flex-col gap-10 lg:w-4/5 m-auto md:w-3/4">
        <div className="grid grid-cols-1 gap-5 mt-5 ">
          ...
          <div className="date-cont c-card   p-5 rounded-md min-h-[50vh] overflow-y-scroll relative">
            <h2 className="text-lg mb-5 text-white font-bold">
              ALL OF EXPENSES
            </h2>

            <p className="text-green-500 font-semibold text-lg  absolute right-0 p-5">
              {expenseDelete ? "Delete Sucessfully : Reloading..." : null}
            </p>
            ...
            <div className="fixed z-20 bottom-0 flex items-center justify-center left-0 right-0 mt-2 w-full ">
              <p className="bg-[#00000035] w-full p-5 text-white font-extrabold text-lg ">
                ...
                {expensesData?.data.map((expense) => {
                  return (
                    <div key={expense._id}>
                      <span>{expense.expense.forEach(expense in expense)}</span>
                    </div>
                  );
                })}
              </p>
        ...
        <div>
          ...
          <div className="c-card   rounded-xl p-5 grid grid-flow-dense lg:grid-cols-3 gap-5 md:grid-cols-2 grid-cols-1 h-[50vh] overflow-y-scroll relative">
            <p className="text-green-500 font-semibold text-lg absolute right-0 p-5">
              {reminderDelete ? "Delete Sucessfully : Reloading..." : null}
            </p>
